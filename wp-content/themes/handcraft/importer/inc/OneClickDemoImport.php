		...
		if ( isset($_GET['page']) && $_GET['page'] === 'octheme_settings') {
			...
			wp_localize_script( 'ocdi-main-js', 'ocdi',
				array(
					...
					'texts'            => array(
						'missing_preview_image' => esc_html__( 'No preview image defined for this import.', 'handcraft' ),
						'dialog_title'          => esc_html__( 'Are you sure?', 'handcraft' ),
						'dialog_no'             => esc_html__( 'Cancel', 'handcraft' ),
						'dialog_yes'            => esc_html__( 'Yes, import!', 'handcraft' ),
						'selected_import_title' => esc_html__( 'Selected demo import:', 'handcraft' ),
					),
	...
	public function import_demo_data_ajax_callback() {
		...
		if ( ! $use_existing_importer_data ) {
			...
			if ( ! empty( $_FILES ) ) { // Using manual file uploads?
				...
				$this->import_files[ $this->selected_index ]['import_file_name'] = esc_html__( 'Manually uploaded files', 'handcraft' );
			}
			elseif ( ! empty( $this->import_files[ $this->selected_index ] ) ) { // Use predefined import files from wp filter: pt-ocdi/import_files.
				...
				if ( is_wp_error( $this->selected_import_files ) ) {
					...
					Helpers::log_error_and_send_ajax_response(
						...
						$this->log_file_path,
						esc_html__( 'Downloaded files', 'handcraft' )
					);
				}
				...
				wp_send_json( esc_html__( 'No import files specified!', 'handcraft' ) );
			}
		}
	...
	private function final_response() {
		...
		if ( empty( $this->frontend_error_messages ) ) {
			...
			$response['message'] .= sprintf(
				__( '%1$s%3$sThat\'s it, all done!%4$s%2$sThe import of demo data has finished. Please check your page and make sure that everything has been imported correctly.%5$s', 'handcraft' ),
			...
			$response['message'] .= sprintf(
				__( '%1$sThe demo import has finished, but there were some import errors.%2$sMore details about the errors can be found in this %3$s%5$slog file%6$s%4$s%7$s', 'handcraft' ),
